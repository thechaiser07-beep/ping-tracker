...
async function atReq(method, table, idOrQuery = '', body = null) {
    ...
    // Query strings start with '?' — don't add a slash before them
    const suffix = idOrQuery ? (idOrQuery.startsWith('?') ? idOrQuery : '/' + idOrQuery) : '';
    const url  = `${AT_URL()}/${encodeURIComponent(table)}${suffix}`;
...
// atUpsert — update if we have a cached ID, fall back to create on 404 (stale cache)
async function atUpsert(table, recKey, fields) {
    const recId = cachedId(recKey);
    if (recId) {
        try {
            await atUpdate(table, recId, fields);
            return;
        } catch (e) {
            const msg = String(e.message);
            if (msg.includes('404') || msg.includes('NOT_FOUND')) {
                // Stale cache — record was deleted in Airtable; clear and fall through to create
                delete REC[recKey];
...
async function enterApp() {
    ...
    renderQuickAdd();
    renderCaff();
    renderCustomCaffButtons();
    renderCustomDrinksList();
    renderWater();
    updateRing();
    renderWaterStats();
    renderSleepLog();
    ...
    // Fetch current month from Airtable + synced settings
    await Promise.all([
        loadMonthData(viewWY, viewWM),
        loadCustomDrinks(),
    ]);
...
async function loadMonthData(y, m) {
    ...
    try {
        syncDot('busy');
        const [wData, sData, cData, mData] = await Promise.all([
        ...
        for (const rec of sData.records) {
            ...
            // Sync tags back from Airtable if present
            if (rec.fields.Tags) {
                sleepTagsData[dt] = rec.fields.Tags.split(',').map(t => t.trim()).filter(Boolean);
                localStorage.setItem('ping_stags', JSON.stringify(sleepTagsData));
            }
        }
        ...
        localStorage.setItem('ping_mood', JSON.stringify(moodData));
...
function renderDashboard() {
    ...
    for (let i = 0; i < 366; i++) {
        ...
        if (e && e.type !== 'nap' && e.duration >= S.sGoal) sStreak++; else break;
...
async function saveWater(entries) {
    ...
    try {
        ...
        await atUpsert('Water', 'w_' + S.today, payload);
        syncDot('ok');
...
async function saveCaff(date) {
    ...
    try {
        ...
        await atUpsert('Caffeine', 'c_' + date, payload);
        syncDot('ok');
...
document.getElementById('log-sleep').addEventListener('click', async () => {
    ...
    const dur   = calcSleep();
    const entry = { date, bedtime: bed, waketime: wake, duration: +dur.total.toFixed(2), quality: sleepQuality, notes, type: sleepType, tags: [...selectedSleepTags] };

    // Save tags separately (survive Airtable re-sync)
    if (selectedSleepTags.length) {
        sleepTagsData[date] = [...selectedSleepTags];
        localStorage.setItem('ping_stags', JSON.stringify(sleepTagsData));
    }
    ...
    if (AT_READY()) {
        syncDot('busy');
        try {
            const fields = { Date: date, Bedtime: bed, Waketime: wake, Duration: entry.duration, Quality: sleepQuality, Notes: notes, Type: sleepType, Tags: (sleepTagsData[date] || []).join(',') };
            try {
                await atUpsert('Sleep', 's_' + date, fields);
            } catch (tagErr) {
                // Tags field may not exist in Airtable yet — retry without it
                if (String(tagErr.message).includes('UNKNOWN_FIELD_NAME') || String(tagErr.message).includes('Unknown field')) {
                    const { Tags, ...baseFields } = fields;
                    await atUpsert('Sleep', 's_' + date, baseFields);
                    toast('Add a Tags field to your Sleep table to sync tags', false);
                } else { throw tagErr; }
            }
            syncDot('ok');
...
function renderSleepLog() {
    ...
    el.innerHTML = S.sleepLogs.slice(0, 14).map(l => {
        ...
        const ltags = sleepTagsData[l.date] || [];
        const tagsHtml = ltags.length ? `<div class="sleep-log-tags">${ltags.map(t => `<span class="sleep-log-tag">${t.replace('-', ' ')}</span>`).join('')}</div>` : '';
        const sched  = getSchedule();
...
// ════════════════════════════════════════════════════
//  v6 — BEDTIME COUNTDOWN
// ════════════════════════════════════════════════════
function renderBedtimeCountdown() {
    const sched = getSchedule();
    const card  = document.getElementById('bedtime-countdown');
    if (!sched.bed) { card.style.display = 'none'; return; }

    const now    = new Date();
    const [bh, bm] = sched.bed.split(':').map(Number);
    let bedMins  = bh * 60 + bm;
    let nowMins  = now.getHours() * 60 + now.getMinutes();
    let diff     = bedMins - nowMins;
    if (diff < 0) diff += 1440;

    // Only show within 4 hours of bedtime
    if (diff > 240) { card.style.display = 'none'; return; }
    card.style.display = '';
    const txt = document.getElementById('countdown-text');
    if (diff <= 0) {
        txt.textContent = 'Time for bed! 🌙';
        card.classList.add('urgent');
    } else {
        const h = Math.floor(diff / 60), m = diff % 60;
        txt.textContent = `Bedtime in ${h > 0 ? h + 'h ' : ''}${m}m`;
        card.classList.toggle('urgent', diff < 30);
    }
}
...
//  v6 — HYDRATION PACING
// ════════════════════════════════════════════════════
function renderHydrationPace() {
    const card   = document.getElementById('pace-card');
    const textEl = document.getElementById('pace-text');
    const now    = new Date();
    const nowMins = now.getHours() * 60 + now.getMinutes();

    const sched = getSchedule();
    let cutoffMins = 21 * 60; // default 9 pm
    if (sched.bed) {
        const [bh, bm] = sched.bed.split(':').map(Number);
        cutoffMins = bh * 60 + bm;
        if (cutoffMins < 12 * 60) cutoffMins += 1440; // next-day bedtime
    }

    const currentMl = todayMl();
    const goalMl    = S.wGoal * S.bSize;

    if (currentMl >= goalMl) {
        textEl.textContent = '✓ Daily goal met — great hydration!';
        card.className = 'pace-card pace-success';
        return;
    }

    let minsLeft = cutoffMins - nowMins;
    if (minsLeft <= 0) minsLeft += 1440;
    if (minsLeft > 14 * 60) minsLeft = 14 * 60; // cap at 14 h ahead

    const mlLeft      = goalMl - currentMl;
    const bottlesLeft = mlLeft / S.bSize;

    if (minsLeft < 20) {
        textEl.textContent = `${bottlesLeft.toFixed(1)} bottles still needed — cutoff soon`;
        card.className = 'pace-card pace-warn';
        return;
    }

    const minsPer = Math.round(minsLeft / bottlesLeft);
    const h = Math.floor(minsPer / 60), m = minsPer % 60;
    const paceStr = h > 0 ? `${h}h ${m}m` : `${m}m`;
    textEl.textContent = `${bottlesLeft.toFixed(1)} left — one every ${paceStr} to hit goal`;
...
//  v6 — SLEEP TAGS
// ════════════════════════════════════════════════════
const sleepTagsData = JSON.parse(localStorage.getItem('ping_stags') || '{}');
...
document.querySelectorAll('.tag-chip').forEach(chip => {
    chip.addEventListener('click', () => {
        ...
        if (chip.classList.contains('active')) {
            if (!selectedSleepTags.includes(tag)) selectedSleepTags.push(tag);
        } else {
            selectedSleepTags = selectedSleepTags.filter(t => t !== tag);
        }
    });
});
...
//  v6 — MOOD LOG
// ════════════════════════════════════════════════════
const moodData = JSON.parse(localStorage.getItem('ping_mood') || '{}');

function renderMoodLog() {
    const todayMood = moodData[S.today];
    const savedEl   = document.getElementById('mood-saved');
    document.querySelectorAll('.mood-btn').forEach(btn =>
        btn.classList.toggle('active', !!(todayMood && parseInt(btn.dataset.score) === todayMood.score))
    );
    if (todayMood) {
        const labels = ['', 'Rough', 'Meh', 'OK', 'Good', 'Great'];
        savedEl.style.display = '';
        savedEl.textContent   = `${labels[todayMood.score]} · ${todayMood.time}`;
    } else {
        savedEl.style.display = 'none';
    }
}

async function saveMood(date) {
    const entry = moodData[date];
    if (!entry || !AT_READY()) return;
    syncDot('busy');
    try {
        await atUpsert('Mood', 'm_' + date, { Date: date, Score: entry.score, Time: entry.time });
        syncDot('ok');
    } catch (e) { syncDot('err'); toast('Mood sync: ' + (e?.message || 'check Settings'), true); }
}

document.querySelectorAll('.mood-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
        const score = parseInt(btn.dataset.score);
        moodData[S.today] = { score, time: hhmm(), ts: Date.now() };
        localStorage.setItem('ping_mood', JSON.stringify(moodData));
        renderMoodLog();
        toast('Mood logged!');
        await saveMood(S.today);
    });
});

// ════════════════════════════════════════════════════
//  v6 — SLEEP COACHING TIPS
// ════════════════════════════════════════════════════
function generateCoachingTips() {
    const tips    = [];
    const entries = Object.entries(monthSleep).filter(([, s]) => s.type !== 'nap' && s.duration > 0);
    if (entries.length < 5) return tips;

    // Weeknight vs weekend sleep difference
    const weeknight = entries.filter(([d]) => { const day = new Date(d + 'T12:00').getDay(); return day >= 1 && day <= 5; });
    const weekend   = entries.filter(([d]) => { const day = new Date(d + 'T12:00').getDay(); return day === 0 || day === 6; });
    if (weeknight.length >= 3 && weekend.length >= 1) {
        const avgWkn = weeknight.reduce((s, [, e]) => s + e.duration, 0) / weeknight.length;
        const avgWkd = weekend.reduce((s, [, e]) => s + e.duration, 0) / weekend.length;
        const diff   = avgWkd - avgWkn;
        if (diff > 0.75) {
            const mins = Math.round((diff / 2) * 60);
            tips.push(`You sleep ${diff.toFixed(1)}h more on weekends. Try moving your weeknight bedtime ${mins}min earlier to close that gap.`);
        }
    }

    // Duration when sleep quality is highest
    const highQ = entries.filter(([, s]) => (s.quality || 0) >= 4);
    if (highQ.length >= 3) {
        const avg = highQ.reduce((s, [, e]) => s + e.duration, 0) / highQ.length;
        const h = Math.floor(avg), m = Math.round((avg % 1) * 60);
        tips.push(`Your best-quality sleep happens around ${h}h${m ? ' ' + m + 'm' : ''} — that's your sweet spot.`);
    }

    // Water–sleep correlation
    const corr = calcCorrelation();
    if (corr && (corr.goalAvgQ - corr.noGoalAvgQ) > 0.3) {
        tips.push(`You rate sleep ${(corr.goalAvgQ - corr.noGoalAvgQ).toFixed(1)} points higher on days you hit your water goal — hydration is paying off.`);
    }

    // High-caffeine days + low sleep quality
    const caffBadDays = Object.entries(monthCaff)
        .filter(([, c]) => c.total > 250)
        .map(([date]) => monthSleep[date])
        .filter(s => s && s.type !== 'nap' && s.quality);
    if (caffBadDays.length >= 3) {
        const avgQ = caffBadDays.reduce((s, e) => s + e.quality, 0) / caffBadDays.length;
        if (avgQ < 3.2) tips.push(`On days with 250mg+ caffeine your sleep quality averages ${avgQ.toFixed(1)}/5 — try an earlier cutoff time.`);
    }
...
function renderCoachingTips() {
    ...
    card.style.display  = '';
    list.innerHTML = tips.map(t => `<div class="coaching-tip">💡 ${t}</div>`).join('');
}

// ════════════════════════════════════════════════════
//  v6 — CUSTOM CAFFEINE PRESETS
// ════════════════════════════════════════════════════
let customDrinks = JSON.parse(localStorage.getItem('ping_caff_custom') || '[]');

async function saveCustomDrinks() {
    localStorage.setItem('ping_caff_custom', JSON.stringify(customDrinks));
    if (!AT_READY()) return;
    try {
        const recId = cachedId('custom_drinks');
        const value = JSON.stringify(customDrinks);
        if (recId) {
            await atUpdate('Settings', recId, { Value: value });
        } else {
            const rec = await atCreate('Settings', { Key: 'custom_drinks', Value: value });
            if (rec) cacheRec('custom_drinks', rec.id);
        }
    } catch { /* non-critical, already saved locally */ }
}

async function loadCustomDrinks() {
    if (!AT_READY()) return;
    try {
        const rec = await atFindSetting('custom_drinks');
        if (rec?.fields?.Value) {
            customDrinks = JSON.parse(rec.fields.Value);
            localStorage.setItem('ping_caff_custom', JSON.stringify(customDrinks));
            if (rec.id) cacheRec('custom_drinks', rec.id);
            renderCustomDrinksList();
            renderCustomCaffButtons();
        }
    } catch { /* non-critical */ }
...
function renderCustomDrinksList() {
    const el = document.getElementById('custom-drinks-list');
    if (!customDrinks.length) {
        el.innerHTML = '<div style="font-size:11px;color:var(--text-dim);margin-bottom:4px">No custom drinks yet</div>';
    } else {
        el.innerHTML = customDrinks.map((d, i) => `
            <div class="custom-drink-item">
                <span>${d.name} — ${d.mg}mg</span>
                <button class="log-del" data-ci="${i}">✕</button>
            </div>`).join('');
    }
}

function renderCustomCaffButtons() {
    const row = document.getElementById('caff-custom-presets');
    if (!row) return;
    row.innerHTML = customDrinks.map(d => `
        <button class="caff-btn" data-mg="${d.mg}" data-name="${d.name}">
            <span>☕</span><small>${d.name.length > 8 ? d.name.slice(0,8) : d.name}<br>${d.mg}mg</small>
        </button>`).join('');
    row.querySelectorAll('.caff-btn').forEach(btn =>
        btn.addEventListener('click', () => logCaff(parseInt(btn.dataset.mg), btn.dataset.name)));
}

document.getElementById('cd-add').addEventListener('click', async () => {
    const name = document.getElementById('cd-name').value.trim();
    const mg   = parseInt(document.getElementById('cd-mg').value);
    if (!name) { toast('Enter a drink name', true); return; }
    if (!mg || mg < 1 || mg > 2000) { toast('Enter 1–2000 mg', true); return; }
    customDrinks.push({ name, mg });
    document.getElementById('cd-name').value = '';
    document.getElementById('cd-mg').value   = '';
    renderCustomDrinksList();
    renderCustomCaffButtons();
    await saveCustomDrinks();
    toast(`${name} (${mg}mg) added!`);
});

document.getElementById('custom-drinks-list').addEventListener('click', async e => {
    const btn = e.target.closest('.log-del');
    if (!btn) return;
    const i = parseInt(btn.dataset.ci);
    const removed = customDrinks.splice(i, 1)[0];
    renderCustomDrinksList();
    renderCustomCaffButtons();
    await saveCustomDrinks();
...
// ════════════════════════════════════════════════════
//  v6 — BACKUP / RESTORE / CSV
// ════════════════════════════════════════════════════
document.getElementById('export-backup').addEventListener('click', () => {
    const data = {};
    for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key.startsWith('ping_')) data[key] = localStorage.getItem(key);
    }
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href     = URL.createObjectURL(blob);
    a.download = `ping-backup-${S.today}.json`;
    a.click();
    ...
    reader.onload = ev => {
        try {
            ...
            Object.entries(data).forEach(([key, val]) => {
                if (key.startsWith('ping_')) { localStorage.setItem(key, val); count++; }
            });
            toast(`Restored ${count} items — reloading…`);
...
document.getElementById('export-csv').addEventListener('click', () => {
    const rows = [['Date', 'Bedtime', 'Waketime', 'Duration (h)', 'Quality', 'Type', 'Notes', 'Tags']];
    Object.entries(monthSleep).sort().forEach(([date, s]) => {
        const tags = (sleepTagsData[date] || []).join(' ');
        rows.push([date, s.bedtime || '', s.waketime || '', s.duration || '', s.quality || '', s.type || '', (s.notes || '').replace(/"/g, '""'), tags]);
    });
    const csv  = rows.map(r => r.map(v => `"${v}"`).join(',')).join('\n');
    const a    = document.createElement('a');
    a.href     = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
    a.download = `ping-sleep-${S.today}.csv`;
    a.click();
    toast('Sleep CSV exported!');
});

// ════════════════════════════════════════════════════
//  v6 — WEEKLY SUMMARY CANVAS
// ════════════════════════════════════════════════════
function openWeeklySummary() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        days.push({
            ...
            water: key === S.today ? todayMl() : (monthWater[key] || 0),
            sleep: monthSleep[key] || null,
            caff:  (monthCaff[key] || {}).total || 0,
            mood:  moodData[key] || null,
        });
    }
    renderWeekCanvas(days);
    document.getElementById('weekly-overlay').classList.add('visible');
}

function renderWeekCanvas(days) {
    ...
    const rng   = `${start.toLocaleDateString('en-US', { month:'short', day:'numeric' })} – ${end.toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' })}`;
    ctx.fillStyle = '#9d4edd'; ctx.font = `bold 15px ${FONT}`; ctx.textAlign = 'center';
    ctx.fillText('PING — WEEKLY SUMMARY', W / 2, 30);
    ctx.fillStyle = '#6666aa'; ctx.font = `10px ${FONT}`;
    ctx.fillText(rng, W / 2, 45);

    const colW   = (W - pad * 2) / 7;
    const barMaxH = 38;

    function drawBars(yTop, color, getValue, metColor) {
        days.forEach((d, i) => {
            const x   = pad + i * colW;
            const pct = getValue(d);
            const bh  = Math.max(2, pct * barMaxH);
            ctx.fillStyle = pct >= 1 ? (metColor || color) : color;
            ctx.fillRect(x + 2, yTop + barMaxH - bh, colW - 6, bh);
            ctx.fillStyle = '#6666aa'; ctx.font = `7px ${FONT}`; ctx.textAlign = 'center';
            ctx.fillText(d.label, x + colW / 2, yTop + barMaxH + 11);
        });
    }

    // Water bars
    let y = 62;
    ctx.fillStyle = '#06d6a0'; ctx.font = `bold 10px ${FONT}`; ctx.textAlign = 'left';
    ctx.fillText('💧 WATER', pad, y);
    const goalMl = S.wGoal * S.bSize;
    drawBars(y + 6, '#06d6a0', d => goalMl > 0 ? Math.min(d.water / goalMl, 1) : 0, '#f72585');
    y += 62;

    // Sleep bars
    ctx.fillStyle = '#9d4edd'; ctx.font = `bold 10px ${FONT}`; ctx.textAlign = 'left';
    ctx.fillText('🌙 SLEEP', pad, y);
    drawBars(y + 6, '#9d4edd', d => d.sleep ? Math.min(d.sleep.duration / S.sGoal, 1) : 0, '#06d6a0');
    y += 62;

    // Mood row
    const moodEmojis = ['', '😴', '😐', '🙂', '😊', '🌟'];
    ctx.fillStyle = '#ffbe0b'; ctx.font = `bold 10px ${FONT}`; ctx.textAlign = 'left';
    ctx.fillText('✨ MOOD', pad, y); y += 14;
    days.forEach((d, i) => {
        const x = pad + i * colW + colW / 2;
        ctx.font = `16px ${FONT}`; ctx.textAlign = 'center';
        ctx.fillText(d.mood ? moodEmojis[d.mood.score] : '·', x, y + 14);
    });
    y += 30;

    // Stats table
    const wDays = days.filter(d => d.water > 0);
    const sDays = days.filter(d => d.sleep && d.sleep.type !== 'nap' && d.sleep.duration > 0);
    const mDays = days.filter(d => d.mood);
    const avgW  = wDays.length ? wDays.reduce((s, d) => s + d.water, 0) / wDays.length : 0;
    const avgS  = sDays.length ? sDays.reduce((s, d) => s + d.sleep.duration, 0) / sDays.length : 0;
    const avgM  = mDays.length ? mDays.reduce((s, d) => s + d.mood.score, 0) / mDays.length : 0;
    const wMet  = days.filter(d => d.water >= goalMl).length;
    const sMet  = sDays.filter(d => d.sleep.duration >= S.sGoal).length;
    const fmtW  = v => v >= 1000 ? (v / 1000).toFixed(1) + 'L' : Math.round(v) + 'ml';
    const fmtS  = v => { const h = Math.floor(v), m = Math.round((v % 1) * 60); return v > 0 ? `${h}h${m ? ' ' + m + 'm' : ''}` : '—'; };

    const stats = [
        ['Avg water / day', fmtW(avgW)],
        ['Water goal days', `${wMet} / 7`],
        ['Avg sleep', fmtS(avgS)],
        ['Sleep goal days', `${sMet} / ${sDays.length}`],
        ['Avg mood', mDays.length ? avgM.toFixed(1) + ' / 5' : '—'],
    ];

    y += 10;
    ctx.fillStyle = '#f0f0ff'; ctx.font = `bold 10px ${FONT}`; ctx.textAlign = 'left';
    ctx.fillText('SUMMARY', pad, y); y += 14;

    stats.forEach(([label, val]) => {
        ctx.fillStyle = '#6666aa'; ctx.font = `10px ${FONT}`; ctx.textAlign = 'left';
        ctx.fillText(label, pad + 6, y);
        ctx.fillStyle = '#f0f0ff'; ctx.font = `bold 10px ${FONT}`; ctx.textAlign = 'right';
        ctx.fillText(val, W - pad, y);
        y += 16;
    });

    // Footer
    ctx.fillStyle = 'rgba(102,102,170,0.4)'; ctx.font = `8px ${FONT}`; ctx.textAlign = 'center';
    ctx.fillText('Generated by PING — Sleep & Water Tracker', W / 2, H - 10);
}
...
document.getElementById('weekly-close').addEventListener('click', () =>
    document.getElementById('weekly-overlay').classList.remove('visible'));
document.getElementById('weekly-overlay').addEventListener('click', e => {
    if (e.target === document.getElementById('weekly-overlay'))
        document.getElementById('weekly-overlay').classList.remove('visible');
});
document.getElementById('download-weekly').addEventListener('click', () => {
    const a = document.createElement('a');
    a.href     = document.getElementById('weekly-canvas').toDataURL('image/png');
    a.download = `ping-weekly-${S.today}.png`;
    a.click();
    toast('Weekly summary saved!');
});
